...
以下功能已经在本地 Docker 中间件环境完成端到端验证。系统不仅提供对话页面，也提供面向运营和研发排障的管理后台，用于观察知识路由、文档处理、索引构建和 Agent 执行轨迹。

### 开放式问答与联网搜索

开放式提问会进入 ReAct Agent 执行路径。对于天气、新闻等具有时效性的问题，Agent 会优先调用 Tavily 联网搜索，并在回答中明确使用当前日期，避免将历史结果误认为实时信息。

聊天页面同时保留会话列表、历史消息、复制回答、新建会话和删除会话等常用交互。联网工具不可用时，系统会展示明确的降级提示，不会编造实时信息。

### 企业文档问答与自动知识路由

用户提出“出差怎么报销”这类企业知识问题时，系统会先执行知识范围预选和文档候选召回，再进入稳定检索链路。管理界面会展示：

- 路由置信度、命中关键词和当前主候选文档；
- 候选文档匹配分与排序结果；
- 高置信直接检索、低置信扩大候选范围、存在歧义时主动追问；
- 基于证据片段生成的结构化回答，以及对应引用编号。

当用户问题存在文档范围歧义时，系统会返回候选文档列表，引导用户明确选择目标文档后再回答。例如，新员工培训问题可以在多个手册候选中继续选择《澄星智能新员工入职培训手册.md》。

### 文档接入与索引构建工作台

管理后台支持上传 `PDF / DOC / DOCX / TXT / MD / HTML` 文档，并通过可视化工作台跟踪完整生命周期：

| 阶段 | 业务说明 |
| --- | --- |
| `00 文档概览` | 查看解析状态、关键指标和当前工作焦点 |
| `01 配置策略` | 调整推荐的 Parent / Child 双流水线切块策略 |
| `02 确认并构建` | 确认方案，触发 Kafka 异步索引任务 |
| `03 验证 Chunk 结果` | 检查父块、子块、Token、向量化状态与正文预览 |
| `04 查看任务记录` | 查看每个阶段的执行日志、耗时和结构化快照 |

切块策略支持按文档结构切块、递归分块、语义分块和大模型智能切块。策略确认后，索引构建会依次执行：

```text
切块执行 -> 切块后处理 -> 向量化 -> 入库完成
```

实测文档索引任务能够完成 PGVector 向量写入，并保留完整阶段轨迹。示例任务生成了 `20` 个父块、`26` 个子块，其中 `20` 个父块可用于向量检索；Chunk 验证页可以继续展开查看子块证据与所属父块上下文。

### 对话观测与问题排查

管理后台提供会话级观测页面，可查看：

- 会话模式、最近一轮状态、消息总数和 Checkpoint 数量；
- 最近用户问题与助手回答；
- 每轮对话的执行路径、耗时、引用和工具调用情况；
- 长期摘要状态，并支持手动触发摘要重建；
- 已完成、失败、停止等不同执行结果。
...
![聊天问答与知识路由](需要的例子演示/截图/03c008b3-c492-417c-ae31-67be3d343153.png)

#### 文档接入与管理

![文档接入与管理](需要的例子演示/截图/0d7a1fe4-21c6-465d-a709-fde1ca1d595e.png)

#### 异步索引构建

![异步索引构建](需要的例子演示/截图/80789796-3aac-444e-b77c-03dfd3319418.png)

#### Chunk 验证与追踪

![Chunk 验证与追踪](需要的例子演示/截图/f9d14115-7882-46d0-bd7b-3ed70fb444aa.png)
