...
import org.Lin.ai.manage.data.SuperAgentKnowledgeScopeNode;
import org.Lin.ai.manage.data.SuperAgentKnowledgeTopicNode;
...
import org.Lin.ai.manage.mapper.SuperAgentKnowledgeScopeNodeMapper;
import org.Lin.ai.manage.mapper.SuperAgentKnowledgeTopicNodeMapper;
import org.Lin.ai.manage.mapper.SuperAgentKnowledgeRouteTraceMapper;
...
import org.Lin.ai.manage.vo.KnowledgeRouteTraceItemVo;
...
public class KnowledgeManageServiceImpl implements KnowledgeManageService {
    ...
    public KnowledgeRouteTracePageVo queryRouteTracePage(KnowledgeRouteTraceQueryDto dto) {
        ...
        LambdaQueryWrapper<org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace> wrapper =
            new LambdaQueryWrapper<org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace>()
                .eq(org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getStatus, BusinessStatus.YES.getCode())
                .orderByDesc(org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getCreateTime,
                    org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getId);
        if (StrUtil.isNotBlank(conversationId)) {
            wrapper.eq(org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getConversationId, conversationId);
        }
        if (StrUtil.isNotBlank(mode)) {
            wrapper.eq(org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getMode, mode);
        }
        if (StrUtil.isNotBlank(routeStatus)) {
            Integer parsedStatus = parseInteger(routeStatus, -1);
            if (parsedStatus > 0) {
                wrapper.eq(org.Lin.ai.manage.data.SuperAgentKnowledgeRouteTrace::getRouteStatus, parsedStatus);
            }
        }
